...
impl Util {
    ...
    pub fn move_window(
        ...
        unsafe {
            let func: libloading::Symbol<unsafe extern "C" fn(*const c_char, i32, i32) -> i32> =
                self.lib.get(b"moveWindows")?;
            let c_w_title: CString = CString::new(w_title).unwrap();
            Ok((*func)(c_w_title.as_ptr(), target_x, target_y))
        }
    }
    ...
    pub fn resize_window(
        ...
        unsafe {
            let func: libloading::Symbol<unsafe extern "C" fn(*const c_char, i32, i32) -> i32> =
                self.lib.get(b"resizeWindows")?;
            let c_w_title: CString = CString::new(w_title).unwrap();
            Ok((*func)(c_w_title.as_ptr(), width, height))
        }
    }
    ...
    pub fn resize_and_move_window(
        ...
        unsafe {
            let func: libloading::Symbol<
                unsafe extern "C" fn(*const c_char, i32, i32, i32, i32) -> i32,
            > = self.lib.get(b"resizeAndMoveWindows")?;
            let c_w_title: CString = CString::new(w_title).unwrap();
            Ok((*func)(
                c_w_title.as_ptr(),
                target_x,
