<template>
  <general-dialog
    ...
    title="修改参数"
    v-model="editDialogForm.dialog.visible"
  >
    <template #element>
      <div class="dialog-form">
        ...
        >
          ...
          <el-form-item label="不在API调试中使用" prop="noTest">
            <el-switch
              v-model="editDialogForm.noTest"
              @change="onNoTestChange"
            />
          </el-form-item>
          <el-form-item label="仅在API调试中使用" prop="onlyTest">
            <el-switch
              v-model="editDialogForm.onlyTest"
              @change="onOnlyTestChange"
            />
          </el-form-item>
          <el-form-item label="显示条件" prop="displayCondition">
            <el-select
              v-model="editDialogForm.displayCondition"
              multiple
              placeholder="选择被依赖的字段，当被依赖字段值为true时本字段才显示"
            >
              <el-option
                v-for="argItem in displayConditionOption"
                :key="argItem.id"
                :label="argItem.name + `(${argItem.label})`"
                :value="argItem.name"
              />
            </el-select>
          </el-form-item>
          ...
          <template v-if="editDialogForm.componentType === 'DirInput'">
            ...
            <el-form-item label="验证路径" prop="component.dirInput.verifyPath">
              <el-switch
                v-model="editDialogForm.component.dirInput.verifyPath"
              />
            </el-form-item>
          ...
          <template v-if="editDialogForm.componentType === 'FileInput'">
            ...
            <el-form-item
              label="验证路径"
              prop="component.fileInput.verifyPath"
            >
              <el-switch
                v-model="editDialogForm.component.fileInput.verifyPath"
              />
            </el-form-item>
            <el-form-item label="是否多选" prop="component.fileInput.multiple">
              <el-switch
                v-model="editDialogForm.component.fileInput.multiple"
              />
            </el-form-item>
            <template v-if="editDialogForm.component.fileInput.multiple">
              <el-form-item label="默认值" prop="component.fileInput.mValue">
                <el-select
                  v-model="editDialogForm.component.fileInput.mValue"
                  multiple
                  allow-create
                  filterable
                ></el-select>
          ...
          <template v-if="editDialogForm.componentType === 'RectInput'">
            ...
            >
              <el-input
                v-model="editDialogForm.component.rectInput.targetSrc"
              />
            </el-form-item>
          ...
          <template v-if="editDialogForm.componentType === 'select'">
            ...
            <template
              v-if="editDialogForm.component.select.optionType === 'array'"
            >
              <el-form-item label="选项" prop="component.select.options">
                <el-select
                  v-model="editDialogForm.component.select.options"
                  multiple
                  filterable
                  allow-create
                ></el-select>
            ...
            <template v-else>
              <el-form-item label="选项" prop="component.select.fnOptionsStr">
                <el-input
                  v-model="editDialogForm.component.select.fnOptionsStr"
                />
              </el-form-item>
            </template>
            <el-form-item
              label="选项分隔符"
              prop="component.select.selectOptionSeparator"
            >
              <el-input
                v-model="editDialogForm.component.select.selectOptionSeparator"
              />
            </el-form-item>
            ...
            <el-form-item
              v-if="editDialogForm.component.select.multiple"
              label="不允许新建"
              prop="component.select.notAllowCreate"
            >
              <el-switch
                v-model="editDialogForm.component.select.notAllowCreate"
              />
            </el-form-item>
            <template v-if="editDialogForm.component.select.multiple">
              <el-form-item label="默认值" prop="component.select.mValue">
                <el-select
                  v-model="editDialogForm.component.select.mValue"
                  multiple
                  :allow-create="
                    !editDialogForm.component.select.notAllowCreate
                  "
                  filterable
                >
                  <template
                    v-if="
                      editDialogForm.component.select.optionType === 'array'
                    "
                  >
                    <el-option
                      v-for="item in editDialogForm.component.select.options"
                      :key="item"
                      :label="item"
                      :value="item"
                    />
                  ...
                  <template v-else>
                    <el-option
                      v-for="item in parseFnStr(
                        editDialogForm.component.select.fnOptionsStr
                      )"
                      :key="item"
                      :label="item"
                      :value="item"
            ...
            <template v-else>
              <el-form-item label="默认值" prop="component.select.value">
                <el-select
                  v-model="editDialogForm.component.select.value"
                  :allow-create="
                    !editDialogForm.component.select.notAllowCreate
                  "
                  filterable
          ...
          <template v-if="editDialogForm.componentType === 'slider'">
            <el-form-item label="最小值" prop="component.slider.range.min">
              <el-input-number
                v-model="editDialogForm.component.slider.range.min"
              />
            </el-form-item>
            <el-form-item label="最大值" prop="component.slider.range.max">
              <el-input-number
                v-model="editDialogForm.component.slider.range.max"
              />
            </el-form-item>
            <el-form-item label="步长" prop="component.slider.range.step">
              <el-input-number
                v-model="editDialogForm.component.slider.range.step"
              />
            </el-form-item>
            <el-form-item label="默认值" prop="component.slider.value">
              <slider-input
                v-model="editDialogForm.component.slider.value"
                :step="editDialogForm.component.slider.range.step"
                :min="editDialogForm.component.slider.range.min"
                :max="editDialogForm.component.slider.range.max"
                :width="100"
                label="默认值"
                hide-label
              />
          ...
          <template v-if="editDialogForm.componentType === 'switch'">
            ...
            <el-form-item label="关闭文本" prop="component.switch.inactiveText">
              <el-input
                v-model="editDialogForm.component.switch.inactiveText"
              />
            </el-form-item>
            <el-form-item label="默认值" prop="component.switch.value">
              <el-switch
                v-model="editDialogForm.component.switch.value"
          ...
          <template v-if="editDialogForm.componentType === 'numberInput'">
            <el-form-item label="默认值" prop="component.numberInput.value">
              <el-input-number
                v-model="editDialogForm.component.numberInput.value"
              />
            </el-form-item>
          </template>
          <template v-if="editDialogForm.componentType === 'numberRangeInput'">
            <el-form-item
              label="范围限制"
              prop="component.numberRangeInput.limit"
            >
              <range-input
                v-model="editDialogForm.component.numberRangeInput.limit"
              />
            </el-form-item>
            <el-form-item
              label="默认值"
              prop="component.numberRangeInput.value"
            >
              <range-input
                v-model="editDialogForm.component.numberRangeInput.value"
  ...
  <div w-full flex flex-row pos-relative h-85vh position-relative>
    <div overflow-y-scroll overflow-x-hidden p-3 box-border class="b1">
      <el-text>预设组件</el-text>
      <el-divider />
      <VueDraggable
        v-model="localArgs"
        :animation="150"
        :group="{ name: 'args', pull: 'clone', put: false }"
        ghostClass="ghost"
        :clone="clone"
        :sort="false"
      >
        <dialog-arg-item
          v-for="argItem in localArgs"
          :key="argItem.id"
          :argItem="argItem"
        />
      </VueDraggable>
...
const displayConditionOption = computed(() => {
  return model.value
    .filter((i) => i.name !== editDialogForm.name)
    .map((i) => {
      return {
        id: i.id,
        name: i.name,
...
const editArg = (id: string) => {
  ...
  if (target) {
    ...
    if (target.componentType === "select") {
      ...
      editDialogForm.component.select.selectOptionSeparator =
        t.selectOptionSeparator || "";
...
const editDialogCallback = () => {
  const target = model.value?.find(
    (i) =>
      i.id === editDialogForm.dialog.targetId &&
