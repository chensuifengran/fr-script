<template>
  <div w-full box-border flex flex-col>
    <div class="form-item">
      <el-text tag="b">API名称</el-text>
      <el-input v-model="localForm.name" placeholder="API名称" style="width: 240px" />
    </div>
    <div class="form-item">
      <el-text tag="b">API状态</el-text>
      <el-switch v-model="localForm.disabled" active-text="禁用" inactive-text="启用"
        style="--el-switch-on-color: #ff4949; --el-switch-off-color: var(--el-color-primary);" />
    </div>
    <div class="form-item">
      <el-text tag="b">命名空间(namespace)</el-text>
      <el-select v-model="localForm.scope" filterable allow-create default-first-option :reserve-keyword="false"
        placeholder="Root" style="width: 240px" @change="selectScopeHandler">
        <el-option v-for="(item, idx) in scopeList" :key="idx" :label="item.label" :value="item.value" />
        <template #label="{ label }">
          <div flex flex-row flex-items-center>
            <el-tag size="small" type="success" mr-1 v-if="!scopeList.find(i => i.label === label)">new</el-tag>
    ...
    <div class="form-item">
      <el-text tag="b">显示权重(越大默认排序越靠前)</el-text>
      <el-input-number v-model="localForm.weight" :min="0" placeholder="显示权重" />
    </div>
    <div flex flex-col>
      <div class="form-item">
        <el-text tag="b">API文档</el-text>
      ...
      <div flex flex-col pl-5 box-border mt-2 v-show="apiFormShow">
        <div flex flex-col flex-items-start mb-2>
          ...
          <el-input v-model="localForm.document.howToUse" type="textarea" placeholder="API使用介绍" autosize />
        </div>
        <div flex flex-col flex-items-start mb-2>
          ...
          <el-table :data="localForm.document.params" table-layout="fixed" row-key="id" default-expand-all
            ...
            <el-table-column prop="required" label="必填" width="70">
              <template #default="scope">
                <el-switch v-model="scope.row.required" active-text="是" inactive-text="否" inline-prompt
                  @change="onRequiredChange(scope.row.id, scope.row)" />
              </template>
            </el-table-column>
            <el-table-column prop="instructions" label="说明" width="260">
              <template #default="scope">
                <el-input v-model="scope.row.instructions" type="textarea" placeholder="说明" size="small" autosize />
              </template>
            </el-table-column>
            <el-table-column prop="type" label="类型" width="200">
              <template #default="scope">
                <el-select v-if="Array.isArray(scope.row.type)" multiple v-model="scope.row.type" filterable
                  allow-create></el-select>
                <el-autocomplete v-else v-model="scope.row.type" :fetch-suggestions="queryTypeSearch" clearable
                  placeholder="请输入类型名称">
                  <template #default="{ item }">
                    <div class="value">{{ item.value }}</div>
                  </template>
                </el-autocomplete>
                <el-checkbox ml-1 :checked="Array.isArray(scope.row.type)" label="枚举" size="small"
            ...
            <el-table-column fixed="right" width="100">
              <template #header>
                <el-button link type="danger" size="large" @click="clearParamLine()">
                  <el-icon size="large"><span i-mdi-sort-variant-remove></span></el-icon>
                </el-button>
                <el-button link type="primary" size="large" @click="newParamLine()">
                  <el-icon size="large"><span i-mdi-playlist-plus></span></el-icon>
                </el-button>
              </template>
              <template #default="scope">
                <div w-full flex flex-row flex-items-center justify-center>
                  <el-button link class="ml-1" type="danger" @click="clearParamLine(scope.row.id)">
                    <el-icon size="large"><span i-mdi-close></span></el-icon>
                  </el-button>
                  <el-button link class="ml-1" type="primary" @click="newParamLine(scope.row.id)">
                    <el-icon size="large"><span i-mdi-filter-variant-plus></span></el-icon>
                  </el-button>
                  <el-button link class="ml-1" @click="sortUp(scope.row.id)">
                    <el-icon size="large"><span i-mdi-arrow-up-thin></span></el-icon>
                  </el-button>
                  <el-button link class="ml-1" @click="sortDown(scope.row.id)">
                    <el-icon size="large"><span i-mdi-arrow-down-thin></span></el-icon>
                  </el-button>
                </div>
              </template>
            </el-table-column>
        ...
        <div flex flex-col flex-items-start mb-2 w-full>
          ...
          <div w-full box-border pl-2>
            <div class="form-item" w-full>
              <el-text>类型</el-text>
              <el-autocomplete v-model="localForm.document.returnValue.type" style="width: 480px"
                :fetch-suggestions="queryTypeSearch" clearable placeholder="请输入类型名称">
                <template #default="{ item }">
                  <div class="value">{{ item.value }}</div>
                </template>
              </el-autocomplete>
            ...
            <div flex flex-col flex-items-start w-full box-border p-8px>
              ...
              <el-input v-model="localForm.document.returnValue.instructions" type="textarea" placeholder="返回值说明"
                autosize />
            </div>
          </div>
        </div>
        <div flex flex-col flex-items-start mb-2 w-full>
          ...
          <div w-full box-border pl-2>
            <div flex flex-col flex-items-start w-full box-border p-8px>
              ...
              <el-input v-model="localForm.document.example.title" type="textarea" placeholder="示例标题" autosize />
            </div>
            <div flex flex-col flex-items-start w-full box-border p-8px>
              <el-text self-start>代码</el-text>
              <!-- TODO: 使用monaco-editor实现，并且将生成的类型声明提供给monaco-editor -->
              <el-input v-model="localForm.document.example.code" type="textarea" placeholder="示例代码" autosize />
            </div>
          </div>
        </div>
        <div flex flex-row flex-items-center w-full justify-between pr-2 mb-2 box-border>
          <el-text>搜索关键词</el-text>
          <el-select multiple filterable allow-create v-model="localForm.document.searchKeys" style="width: 480px"
    ...
    <div flex flex-col>
      <div class="form-item">
        <el-text tag="b">测试弹窗/快速修改参数弹窗</el-text>
      ...
      <div flex flex-col pl-5 box-border v-show="dialogFormShow" w-full pr-2>
        <div class="form-item">
          ...
          <el-switch v-model="localForm.dialog.notOpen" active-text="是" inactive-text="否" />
        </div>
        <div v-if="!localForm.dialog.notOpen">
          <div class="form-item">
            <el-text>在API测试窗口显示的名称</el-text>
            <el-input v-model="localForm.dialog.title" placeholder="在API测试窗口显示的名称" style="width:240px" />
          </div>
          <div flex flex-col flex-items-start w-full box-border pl-8px>
            <el-text self-start>弹窗中显示的内容(用于对API进行介绍或者说明)</el-text>
            <el-input v-model="localForm.dialog.content" autosize placeholder="在API测试窗口显示的名称" type="textarea" />
          </div>
          <div flex flex-col flex-items-start w-full box-border pl-8px>
...
const onTypeChange = (val: CheckboxValueType, id: string) => {
  const target = dataStructureUtils.findByIdInTree(id, localForm.value.document.params!);
  if (target) {
    if (!val) {
...
const queryTypeSearch = (queryString: string, cb: any) => {
  const results = queryString
    ? typeList.filter(i => i.toLocaleLowerCase().includes(queryString.toLocaleLowerCase()))
    : typeList;
  const s = `'${queryString}'`;
...
const newParamLine = (parentId?: string) => {
  if (parentId === undefined) {
    ...
    const target = dataStructureUtils.findByIdInTree(parentId, localForm.value.document.params!);
    if (target) {
      if (target.children) {
...
const sortUp = (id: string) => {
  const parent = dataStructureUtils.getTreeParentById(
    id, localForm.value.document.params!
  );
  if (parent) {
...
const clearParamLine = async (id?: string) => {
  if (id === undefined) {
    ...
    try {
      ...
      parent!.splice(parent.findIndex(p => p.id === id), 1);
      ElMessage.info('删除完成');
    } catch (error) {
    }
  }
