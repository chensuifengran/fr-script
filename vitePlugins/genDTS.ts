...
const syncBuildFormDeclare = (typesDir: string, invokesDir: string) => {
  ...
  const oPath = resolve(invokesDir, "buildFormDeclare.ts");
  if (existsSync(oPath)) {
    ...
    if (oldContent.trim() === res.trim()) {
      console.log(
        "✨",
        "The",
        chalk.green("buildFormDeclare.ts"),
        "file is not changed"
      );
      return;
    }
...
const syncRendererListDeclare = (typesDir: string, invokesDir: string) => {
  ...
  const oPath = resolve(invokesDir, "rendererListDeclare.ts");
  if (existsSync(oPath)) {
    const oldContent = readFileSync(oPath, "utf-8");
    if (oldContent.trim() === res.trim()) {
      console.log(
        "✨",
        "The",
        chalk.green("rendererListDeclare.ts"),
        "file is not changed"
      );
      return;
    }
